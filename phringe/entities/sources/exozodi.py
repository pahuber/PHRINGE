...
class Exozodi(BaseSource):
    ...
    def _sky_coordinates(self):
        ...
        return self._get_cached_value(
            ...
            required_attributes=(
                self._grid_size,
                self._instrument.field_of_view
            )
        )

    def _get_sky_coordinates(self) -> Coordinates:
        sky_coordinates = torch.zeros((2, len(self._instrument.field_of_view), self._grid_size, self._grid_size),
                                      dtype=torch.float32, device=self._device)

        # The sky coordinates have a different extent for each field of view, i.e. for each wavelength
        for index_fov in range(len(self._instrument.field_of_view)):
            sky_coordinates_at_fov = get_meshgrid(
                self._instrument.field_of_view[index_fov],
                self._grid_size)
    ...
    def _get_solid_angle(self) -> float:
        ...
        """
        return self._instrument.field_of_view ** 2
    ...
    def _get_spectral_energy_distribution(self) -> np.ndarray:
        field_of_view_in_au = self._instrument.field_of_view * self._host_star_distance * 6.68459e-12
        num_wavelengths = len(self._instrument.field_of_view)
        ...
        for ifov, fov in enumerate(self._instrument.field_of_view):
